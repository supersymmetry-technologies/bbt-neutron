...
传统的大语言模型一直存在数值处理的问题，这个局限性极大地限制了通用大模型在处理真实世界复杂科学问题的能力。因此，我们需要一个全新的科学计算领域的任务范式，使得我们能够在实际科学实验数据分析的场景下能够进行上下文学习以及挖掘潜在的scaling law. 

为了解决这个问题，我们开发了Binary Big Bang Transformer (BBT-Neutron)，一个转为科学领域设计的多任务大语言模型。模型在对数字分词的环节中创新性地采用了二进制分词方法。与传统大语言模型的分词方法不同，我们将输入数据以UTF-8编码方式转换为字节序列。这样既保留了数值内部的结构，又避免了传统BPE分词方法在数值问题上的歧义性。

Jet来源鉴别问题(JoI)是在高能粒子物理对撞机实验中的一个很重要且有挑战性的问题。我们的BBT-Neutron在Jet来源鉴别问题上的结果已经达到了领域内最好水平。
