...
def main():
    ...
    file_handler = logging.FileHandler(os.path.join(log_dir, f"finetune_lora.log"))
    ...
    class PrinterCallback(TrainerCallback):
        ...
        def on_save(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
            ...
            epoch = state.epoch
            num_input_tokens_seen = state.num_input_tokens_seen
            if state.is_local_process_zero:
                logger.info(f"saved when global_step:{global_step}, epoch:{epoch}, num_input_tokens_seen:{num_input_tokens_seen}")
    ...
    class BinDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            batch = dict()
            padded = self.tokenizer.pad(features,
                                        padding="longest",
